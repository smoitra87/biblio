		...
		\For{each splitting position $k$}
			...
			\For{each competitor $t$ at $i$}
				...
				\For{each partition $v$ at $k$}
					\If{$X+\left[E_{ik}(r_i^r,r_t^v) - E_{ik}(r_i^t,r_k^v)\right] > 0$}
...
%\definecolor{bg}{rgb}{0.95,0.95,0.90}
%\begin{minted}[bgcolor=bg,linenos=true,mathescape]{python}
%class A(object) : 
%	""" This is class A """
%	# This calculates $\sum_{j<i} \min_X \left[ E_{ij}(r_i^A,r_j^X) \right]$
%	def __init__(self) : 
%		a = "An object"
%	def print_a(self) : 
%		print self.a
